...
inline bool Server::write_response(Stream &strm, bool close_connection,
  ...
  // Response line
  if (!bstrm.write_format("HTTP/1.0 %d %s\r\n", res.status,
...
inline bool ClientImpl::read_response_line(Stream &strm, Response &res) {
  ...
  if (!line_reader.getline()) { printf("Readline return no data\n"); return false; }

  const static std::regex re("(HTTP/1\\.[01]) (\\d+) (.*?)\r\n");

  std::cmatch m;
  if (!std::regex_match(line_reader.ptr(), m, re)) {printf("Regex_match HTTP failed\n"); return false; }
  ...
  while (res.status == 100) {
    if (!line_reader.getline()) {printf("Read(status=100) return CRLF\n"); return false; } // CRLF
    if (!line_reader.getline()) {printf("Read(status=100)  next response Failed\n"); return false; } // next response line
...
inline bool ClientImpl::handle_request(Stream &strm, const Request &req,
  ...
  if (!ret) { printf("Process_request ret 0\n"); return false; }

  if (300 < res.status && res.status < 400 && follow_location_) {
    printf("300 < res status < 400 redirect now\n");
...
inline bool ClientImpl::process_request(Stream &strm, const Request &req,
  ...
  if (!read_response_line(strm, res) ||
    ...
    printf("Read response headers false\n");
    error_ = Error::Read;
    return false;
  }
  // printf("read response header ok\n");
